import { useQuery } from "@tanstack/react-query";
import { Link } from "react-router-dom";
import { salesApi } from "../api/sales-api";
import { partnersApi } from "@/features/partners/api/partners-api";
import { Button } from "@/components/ui/Button";
import { Badge } from "@/components/ui/Badge";
import { Table, TableHead, TableBody, Th, Td, EmptyRow } from "@/components/ui/Table";
import { formatCurrency, formatDate } from "@/lib/format";

export function SaleListPage() {
  const { data: sales, isLoading } = useQuery({ queryKey: ["sales"], queryFn: salesApi.list });
  const { data: partners } = useQuery({ queryKey: ["partners", undefined], queryFn: () => partnersApi.list() });

  function partnerName(id: number | null) {
    if (id === null) return "Consumidor final";
    return partners?.find((p) => p.id === id)?.name ?? `#${id}`;
  }

  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-slate-800">Ventas</h1>
        <Link to="/sales/new">
          <Button>Nueva venta</Button>
        </Link>
      </div>

      <Table>
        <TableHead>
          <tr>
            <Th>Fecha</Th>
            <Th>Cliente</Th>
            <Th>Items</Th>
            <Th>Pago</Th>
            <Th>Total</Th>
          </tr>
        </TableHead>
        <TableBody>
          {isLoading ? (
            <EmptyRow colSpan={5} label="Cargando..." />
          ) : !sales || sales.length === 0 ? (
            <EmptyRow colSpan={5} label="Aun no hay ventas registradas" />
          ) : (
            sales.map((s) => (
              <tr key={s.id}>
                <Td>{formatDate(s.date)}</Td>
                <Td>{partnerName(s.partner_id)}</Td>
                <Td className="text-slate-500">{s.items.length}</Td>
                <Td>
                  <Badge tone={s.payment_method === "CONTADO" ? "green" : "amber"}>
                    {s.payment_method}
                  </Badge>
                </Td>
                <Td className="font-medium">{formatCurrency(s.total)}</Td>
              </tr>
            ))
          )}
        </TableBody>
      </Table>
    </div>
  );
}
